  ...
  switchAccordion = function(e) {
    ...
    if(thisAnswer.classList.contains('is-collapsed')) {
      for (var i=0,len=accordionToggles.length; i<len; i++) {
        setAccordionAria(accordionToggles[i], accordionToggles[i].parentNode.nextElementSibling, 'false');
        accordionToggles[i].classList.remove('is-collapsed');
        accordionToggles[i].classList.remove('is-expanded');
        accordionToggles[i].parentNode.nextElementSibling.classList.remove('is-expanded');
        accordionToggles[i].parentNode.nextElementSibling.classList.remove('animateIn');
        accordionToggles[i].parentNode.nextElementSibling.classList.add('is-collapsed');
      }
...
var sortByFriendsCount = _.flatMap(_.sortBy(dataObj, 'friends'), 'name');
  console.log('sortByFriendsCount: ', sortByFriendsCount);
/*3 task*/
sortedByFriendsNames = _.uniq(_.flatMap(_.flatMap(dataObj, 'friends'), 'name'));
  console.log('sortedByFriendsNames: ', sortedByFriendsNames);
